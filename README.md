...
6. **RAG 기반 하이브리드 검색 (RAG-based Hybrid Search)**
   사용자의 자연어 질의를 더 깊이 이해하고 정확한 결과를 반환하기 위해 Qdrant(벡터 DB)와 OpenSearch(키워드 검색)를 결합한 하이브리드 검색(RAG)을 지원합니다. OpenAI의 `text-embedding-3-small` 모델을 사용하여 상품, 리뷰, 정책 데이터를 임베딩하고, 각 도메인 특성에 맞는 검색 전략(키워드 우선, 벡터 우선, 균형)을 적용합니다.
   * **동작 방식**: 실시간으로 입력되는 상품과 리뷰 데이터를 자동 임베딩 파이프라인이 처리하여 Qdrant와 OpenSearch에 적재하며, 에이전트가 상황에 맞는 RAG 도구를 호출하여 최적의 검색 결과를 생성합니다.
   * **기대 효과**: "여름에 입기 좋은 시원한 셔츠"와 같은 추상적인 질의부터 특정 약관에 대한 세부 질문까지 사람처럼 이해하고 정확히 답변할 수 있습니다.

### 시스템의 차별점

* **하이브리드 검색 (Hybrid Search)**: 단순 키워드 매칭의 한계를 넘어, 키워드 검색(OpenSearch)과 시맨틱 벡터 검색(Qdrant)을 결합한 하이브리드 RAG 아키텍처를 도입하여 사용자의 의도를 정확하게 파악하고 관련성 높은 정보를 제공합니다.
...
|  포트: 80 (단일 진입점, Spring Cloud Gateway 기반 라우팅, Redis Rate Limiting, X-Request-ID 발급, CORS)|
+-----------------------------------------------------------------------------------+
                                         |
                  +----------------------+----------------------+
                  | (Agent 요청)                                | (Microservice 직접 요청)
                  v                                             v
+---------------------------------------------------+ +---------------------------------------------------+
|          Agent Service (FastAPI / Python)         | |           Spring Boot Microservices (Java)        |
|  포트: 8000                                       | |  (각 서비스는 독립적으로 배포되며, 자신만의 DB를 가짐)|
|                                                   | |                                                   |
|  +---------------------------------------------+  | |  +---------------+ +---------------+              |
|  |             LangGraph Supervisor            |  | |  |product-service| |review-service |              |
|  | (의도 파악 및 가장 적합한 에이전트에게 위임)|  | |  | 포트: 8081    | | 포트: 8082    |              |
|  +---------------------------------------------+  | |  +---------------+ +---------------+              |
|          |           |            |          |    | |  +---------------+ +---------------+              |
|  +-------v------+ +--v----+ +-----v----+ +---v--+ | |  | order-service | |inventory-service|            |
|  |product_search| |review | |cart_mgnt | | ...  | | |  | 포트: 8083    | | 포트: 8084    |              |
|  +--------------+ +-------+ +----------+ +------+ | |  +---------------+ +---------------+              |
|          |           |            |          |    | |  +---------------+                              |
|          +-----+-----+------------+----------+    | |  |payment-service|                              |
|                |                                  | |  | 포트: 8085    |                              |
|        +-------v-------+                          | |  +---------------+                              |
|        |     Tools     | (22개의 외부 API 도구)   | |                                                   |
|        +---------------+                          | +---------------------------------------------------+
|                |                                  |          |                        |
|        +-------v-------+                          |          |                        | Kafka Events
|        |    Reflect    | (오류 발생 시 재시도)    | <--------+ REST API (HTTPX 호출)  v
|        +---------------+                          |                           +-----------------------+
|                |                                  |                           |    Confluent Kafka    |
|        +-------v-------+                          | <-----------------------+ |      & Zookeeper      |
|        |    RAG 도구   | (하이브리드 검색)        |      (검색 결과 제공)       +-----------------------+
|        +---------------+                          |                                       |
+---------------------------------------------------+                                       | 
                         |                                                                  | 
                         v                                                                  v
+---------------------------------------------------+ +---------------------------------------------------+
|     Qdrant (Vector DB) & OpenSearch (Keyword DB)  | |    Embedding Pipeline (Python Kafka Consumer)     |
|     (상품, 리뷰, 정책 등 데이터 적재 및 검색)     | |    (상품/리뷰 데이터 실시간 임베딩 생성 및 적재)  |
+---------------------------------------------------+ +---------------------------------------------------+
                                 ^                                                          |
                                 |               (임베딩 및 적재)                           |
                                 +----------------------------------------------------------+
...
* **OpenAI gpt-5-mini**: 빠르고 비용 효율적이면서도 뛰어난 추론 능력을 갖춘 기본 LLM입니다.
* **openai**: `text-embedding-3-small` 모델을 이용한 고품질 벡터 임베딩 생성을 담당합니다.
* **qdrant-client**: 벡터 데이터베이스(Qdrant)와의 비동기 통신을 통해 시맨틱 검색을 수행합니다.
* **opensearch-py**: OpenSearch와의 통신을 통해 한국어 형태소 분석 기반의 키워드 검색을 수행합니다.
...
### 인프라 (Infrastructure)
* **Spring Cloud Gateway 4.1.x**: Java/Spring Boot 기반의 API Gateway로서 단일 진입점 역할, 라우팅, Redis 기반 Rate Limiting, CORS, 글로벌 필터를 수행합니다.
* **Qdrant v1.7.4**: 하이브리드 RAG 검색을 위한 초고속 벡터 데이터베이스입니다.
* **OpenSearch v2.11.1**: 한국어 형태소 분석(`korean_analyzer`) 기반의 강력한 키워드 검색 엔진입니다.
...
* **product_search (상품 검색 에이전트)**
  ...
  * **지침**: 예산, 배송 조건, 브랜드 제약을 엄격하게 고려하십시오. 호환성이 필요한 상품(예: 스마트폰과 케이스)의 경우 교차 검증을 수행하십시오. RAG 기반의 키워드 우선 하이브리드 검색(`rag_search_products`)을 활용하여 상품명과 브랜드에 대한 정확한 매칭을 수행하되, 검색 결과가 없을 경우 조건을 스스로 완화하여 재검색을 시도하십시오.

* **review_analysis (리뷰 분석 에이전트)**
  * **역할**: 상품의 리뷰를 분석하여 사용자에게 유용한 인사이트를 제공합니다.
  * **지침**: 단순한 평점 나열을 피하고, 사이즈, 품질, 내구성 등에 대한 구체적인 인사이트를 도출하십시오. RAG 기반의 벡터 우선 시맨틱 검색(`rag_search_reviews`)을 활용하여 주관적인 의견이나 추상적인 표현(예: "재질이 부드러운가요?")을 정확히 찾아내십시오. 검증된 구매자의 리뷰를 우선적으로 참고하며, 부정적인 의견도 숨기지 말고 공정하게 전달하십시오.
...
* **customer_service (고객센터 에이전트)**
  ...
  * **지침**: 사용자의 주문 내역을 정확히 조회하십시오. 취소 요청 시 현재 주문 상태를 확인하고, 이미 배송이 시작되어 취소가 불가능한 경우 자연스럽게 반품 및 환불 절차로 대안을 안내하십시오. 스토어 정책(환불 규정, 배송 정책 등)에 대한 질문이 들어오면 균형 잡힌 하이브리드 검색(`rag_search_policies`)을 통해 정확한 규정을 찾아 안내하십시오.

### 에이전트별 도구(Tool) 바인딩
각 에이전트는 자신의 역할에 필요한 도구만 사용할 수 있도록 제한됩니다.
* **product_search**: `PRODUCT_TOOLS` (검색, 상세조회, 카테고리), `INVENTORY_TOOLS` (재고 확인), `RAG_PRODUCT_TOOLS` (하이브리드 상품 검색)
* **review_analysis**: `REVIEW_TOOLS` (리뷰 조회, 요약, 검색), `PRODUCT_TOOLS` (상품 상세 확인용), `RAG_REVIEW_TOOLS` (시맨틱 리뷰 검색)
* **cart_management**: `CART_TOOLS` (장바구니 생성, 추가, 삭제, 변경), `PRODUCT_TOOLS`, `INVENTORY_TOOLS`, `RAG_PRODUCT_TOOLS`
* **order_management**: `ORDER_TOOLS` (주문 상세, 체크아웃, 승인)
* **customer_service**: `ORDER_TOOLS` (주문 조회, 취소, 환불), `PRODUCT_TOOLS`, `RAG_POLICY_TOOLS` (정책 하이브리드 검색)
...
에이전트는 총 22개의 도구를 사용하여 외부 마이크로서비스 및 검색 엔진과 상호작용합니다. 모든 도구는 비동기(`async`)로 동작하며, 실패 시 자동 재시도 로직이 적용되어 있습니다.
...
| 도구 이름 | 설명 | 파라미터 (이름, 타입, 필수여부) | 반환 타입 |
|---|---|---|---|
| `rag_search_products` | 하이브리드 검색(키워드 우선)을 사용하여 사용자의 의도에 맞는 상품을 정확하게 찾아냅니다. | `query`(str, Y), `category`(str, N), `brand`(str, N), `min_price`(int, N), `max_price`(int, N) | `dict` |
| `rag_search_reviews` | 벡터 우선 시맨틱 검색을 통해 특정 상품의 리뷰 중 사용자의 질문과 의미적으로 일치하는 내용을 찾아냅니다. | `query`(str, Y), `product_id`(str, N), `min_rating`(int, N), `verified_only`(bool, N) | `dict` |
| `rag_search_policies` | 균형 잡힌 하이브리드 검색(키워드+벡터)을 통해 스토어의 교환/환불, 배송, 멤버십 등 각종 정책을 찾아냅니다. | `query`(str, Y), `category`(str, N) | `dict` |

---
## RAG 시스템 아키텍처 (Hybrid Search)

사용자의 자연어 질의를 정확하게 이해하고 관련성 높은 정보를 제공하기 위해 3가지 도메인(Product, Review, Policy)에 대해 RAG 시스템을 구축했습니다.

* **Vector DB**: Qdrant (v1.7.4)
  * `products`, `reviews`, `policies` 3개의 컬렉션을 운영합니다.
  * Cosine Distance 매트릭스와 1536 차원을 사용합니다.
* **Keyword Search**: OpenSearch (v2.11.1)
  * `products`, `policies` 2개의 인덱스를 운영합니다 (한국어 분석기 `korean_analyzer` 적용).
* **Embedding Model**: OpenAI `text-embedding-3-small`
  * 임베딩 텍스트 포맷:
    * Products: `상품명 + 상세 설명 + 브랜드`
    * Reviews: `리뷰 제목 + 리뷰 내용`
    * Policies: `정책 제목 + 정책 내용`
* **Point ID 전략**: `abs(hash(string_id)) % (2^63)`를 사용하여 고유 ID를 생성합니다.

### 하이브리드 검색 전략 (Hybrid Search Strategies)
도메인별 특성에 맞춰 서로 다른 검색 전략을 채택했습니다.
1. **Product (키워드 우선)**: OpenSearch를 Primary로, Qdrant를 Secondary로 활용하며 상품 ID 기준으로 중복을 제거(Dedup)합니다. 상품명, 브랜드 등 정확한 명칭 매칭이 중요하기 때문입니다.
2. **Review (벡터 우선)**: Qdrant를 통한 시맨틱 검색만 수행하며 필터링(별점, 검증된 구매자 등)을 결합합니다. 리뷰는 주관적이고 추상적인 표현이 많아 의미 기반 검색이 효과적입니다.
3. **Policy (균형 잡힌 하이브리드)**: 키워드 검색과 벡터 검색을 병렬로 수행하고 0.5/0.5 비율로 정규화된 점수(Normalized Scoring)를 합산하여 최적의 결과를 도출합니다. 정책은 정확한 용어 매칭과 개념적 이해가 모두 필요합니다.

---

## 자동 임베딩 파이프라인 (Auto Embedding Pipeline)

백엔드 마이크로서비스에서 발생하는 데이터 변경사항을 실시간으로 벡터 DB 및 검색 엔진에 동기화합니다.

* **구현체**: Python 기반 `EmbeddingPipeline` (Kafka Consumer)
* **구독 토픽**: `product.events`, `review.events`
* **트리거 이벤트**: `ProductCreatedEvent`, `ProductUpdatedEvent`, `ReviewCreatedEvent`
* **동작 흐름**:
  * **Product Handler**: 이벤트 수신 → OpenAI API를 통해 임베딩 생성 → Qdrant에 벡터 데이터 Upsert 및 OpenSearch에 키워드 문서 Indexing
  * **Review Handler**: 이벤트 수신 → OpenAI API를 통해 임베딩 생성 → Qdrant에 벡터 데이터 Upsert
* **특징**: 메인 에이전트 서비스와 분리된 독립적인 Kafka 컨슈머 그룹(`agent-service-embedding-pipeline`)을 사용하며, 백그라운드 비동기(asyncio) 태스크로 동작하여 에이전트 응답에 영향을 주지 않습니다. 안전한 종료(Graceful Shutdown)를 지원합니다.

---

## API Gateway (단일 진입점)

다양한 마이크로서비스로 분산된 요청을 통합하고 안정적인 클라이언트 연동을 제공하기 위해 API Gateway를 도입했습니다.

* **인프라**: Spring Cloud Gateway (Spring Boot 3.2.8, Spring Cloud 2023.0.3)
* **단일 진입점**: 포트 `80`을 통해 모든 API 요청을 수신합니다.
* **주요 기능**:
  * **라우팅**: 클라이언트의 요청 경로에 따라 적절한 마이크로서비스로 트래픽을 전달합니다.
  * **Rate Limiting**: Redis(RedisRateLimiter)를 활용하여 IP당 초당 10회(10 req/s)의 요청 제한을 두어 시스템을 보호합니다.
  * **분산 추적**: 모든 요청에 `X-Request-ID`를 발급/전파하여 OpenTelemetry를 통한 전체 구간 추적을 지원합니다.
  * **타임아웃 처리**: LLM 호출 및 에이전트 루프 처리에 시간이 소요될 수 있으므로 `spring.cloud.gateway.httpclient.response-timeout`을 통해 `120초`의 넉넉한 읽기 타임아웃을 설정했습니다.
  * **오류 처리**: 커스텀 `ErrorWebExceptionHandler`를 통해 서비스 다운 시 503 상태 코드와 함께 클라이언트 친화적인 커스텀 JSON 에러 응답을 반환합니다.
  * **CORS 처리**: `CorsWebFilter`를 통한 글로벌 CORS 설정.
  * **요청 로깅**: `GlobalFilter`를 통한 요청/응답 로깅 (메서드, 경로, 상태코드, 응답시간).

### 라우팅 테이블
* `/api/chat` → `agent-service:8000`
* `/api/approve` → `agent-service:8000`
* `/api/products` → `product-service:8081`
* `/api/categories` → `product-service:8081`
* `/api/reviews` → `review-service:8082`
* `/api/orders` → `order-service:8083`
* `/api/inventory` → `inventory-service:8084`
* `/api/payments` → `payment-service:8085`
* `/health` → `agent-service:8000`
...
전체 시스템은 `docker-compose.yml`을 통해 API Gateway, Agent Service, 5개의 Spring Boot 마이크로서비스, 그리고 인프라(Kafka, Qdrant, OpenSearch, Redis, DB 등)를 포함하여 16개 이상의 컨테이너로 완벽하게 오케스트레이션됩니다.
...
### 접속 정보
* **API Gateway (단일 진입점)**: `http://localhost` (포트 80)
* **Agent API (FastAPI Swagger UI)**: `http://localhost/docs` (Gateway 경유)
* **Jaeger UI (분산 추적)**: `http://localhost:16686`
* **Kafka UI (선택적 설치 시)**: `http://localhost:8080`

### 테스트 예시 (cURL)

모든 요청은 80번 포트(API Gateway)를 통해 수행됩니다.

**1. 상품 검색 테스트 (백엔드 우회 호출)**
```bash
curl -X GET "http://localhost/api/products?keyword=셔츠"
```
...
```bash
curl -X POST "http://localhost/api/approve" \
     -H "Content-Type: application/json" \
     -d '{
...
│       │   └── tools.py             # 22개 LangChain 도구(Tool) 구현체
│       ├── agents/
...
│       ├── rag/                     # RAG 하이브리드 검색 모듈
│       │   ├── __init__.py
│       │   ├── embeddings.py         # OpenAI text-embedding-3-small 벡터 생성
│       │   ├── qdrant_store.py       # Qdrant 벡터 DB 클라이언트
│       │   ├── opensearch_store.py   # OpenSearch 키워드 검색 클라이언트
│       │   ├── product_rag.py        # 상품 하이브리드 검색 (키워드 우선)
│       │   ├── review_rag.py         # 리뷰 시맨틱 검색 (벡터 우선)
│       │   ├── policy_rag.py         # 정책 균형 검색 (키워드+벡터 0.5/0.5)
│       │   └── pipeline.py           # 자동 임베딩 파이프라인 (Kafka 컨슈머)
│       └── observability/
...
│       ├── settings.gradle
│       ├── Dockerfile              # gradle:8.10-jdk17 멀티스테이지 빌드
│       └── src/main/java/com/shopping/gateway/
│           ├── GatewayApplication.java
│           ├── config/
│           │   ├── RateLimiterConfig.java    # Redis 기반 IP별 Rate Limiting
│           │   ├── CorsConfig.java           # 글로벌 CORS 설정
│           │   └── GlobalErrorHandler.java   # JSON 에러 응답 핸들러
│           └── filter/
│               ├── RequestIdFilter.java      # X-Request-ID 생성/전파 필터
│               └── LoggingFilter.java        # 요청/응답 로깅 필터
├── infra/                           # 인프라스트럭처 설정 파일
...
└── tools/seed/                      # 초기 데이터 시드 스크립트
    ├── postgres/                    # 데이터베이스 초기 시드(Seed) SQL 스크립트
    ...
    ├── qdrant/
    │   └── mock_policies.py         # 10개 한국어 정책 문서 (환불, 배송, 교환 등)
    └── seed_rag.py                  # Qdrant + OpenSearch 시드 스크립트 (상품 18개, 리뷰 22개, 정책 10개)
```
...
* **결제 게이트웨이 규칙**: `payment-service`는 결제 금액을 100으로 나눈 나머지에 따라 결과를 결정합니다.
  ...
  * 그 외: 결제 승인 (Approve)
* **스토어 정책 (10개 문서)**: 하이브리드 RAG 검색을 테스트하기 위한 쇼핑몰 규정입니다. 환불 규정, 배송 정책, 교환 절차, 멤버십 등급, 제품 보증, 개인정보 처리방침, 이용약관, 고객센터 운영시간, 결제 수단, 배송 추적 안내가 포함됩니다.
* **RAG 시드 스크립트 (`seed_rag.py`)**: `docker-compose` 환경 내에서 실행되며, 위에서 언급된 상품, 리뷰, 정책 데이터를 임베딩하여 Qdrant와 OpenSearch에 자동으로 적재합니다.
...
| `OPENAI_MODEL` | `gpt-5-mini` | 에이전트가 사용할 LLM 모델명 |
| `OPENAI_EMBEDDING_MODEL` | `text-embedding-3-small` | RAG 파이프라인에서 사용할 임베딩 모델명 |
...
| `QDRANT_HOST` | `qdrant` | Qdrant 벡터 DB 접속 주소 |
| `QDRANT_PORT` | `6333` | Qdrant 벡터 DB 접속 포트 |
| `OPENSEARCH_HOST` | `opensearch` | OpenSearch 접속 주소 |
| `OPENSEARCH_PORT` | `9200` | OpenSearch 접속 포트 |
...
8. **키워드와 벡터 하이브리드 검색 전략**
   * **결정**: 상품은 키워드 우선(OpenSearch), 리뷰는 벡터 우선(Qdrant), 정책은 균형 잡힌 방식(0.5/0.5 비율)으로 도메인별 맞춤형 RAG 검색 전략을 적용했습니다.
   * **이유**: 상품은 정확한 명칭과 브랜드 매칭이 필수적이고, 리뷰는 "부드럽다", "시원하다" 같은 주관적이고 의미적인 표현을 이해해야 하며, 정책은 정확한 용어와 전반적인 개념 파악이 모두 필요하기 때문입니다.

9. **API Gateway 단일 진입점**
   * **결정**: Spring Cloud Gateway(Java/Spring Boot 기반)를 사용해 모든 클라이언트 요청이 포트 80을 통해 들어오도록 단일 진입점을 구축했습니다.
   * **이유**: 프로그래매틱한 라우팅 설정, Redis 기반 Rate Limiting, 커스텀 GlobalFilter를 통한 X-Request-ID 및 로깅 처리, 그리고 리액티브 논블로킹 아키텍처를 적용하기 위함입니다. 또한 Spring 생태계(Micrometer Tracing, Actuator 등)와의 완벽한 통합이 가능하여 향후 인증 계층 추가에도 매우 유리합니다.
...
1. **보안 및 인증 (Security & Authentication)**
   ...
   * **개선 계획**: 현재 구축된 Spring Cloud Gateway에 Spring Security와 OAuth2/JWT 기반의 강력한 인증 및 인가(Authorization) 계층을 추가할 예정입니다.

2. **RAG 검색 정확도 튜닝 및 임베딩 모델 고도화**
   * 현재 Qdrant와 OpenSearch를 활용하여 상품, 리뷰, 정책 도메인에 대한 하이브리드 RAG 검색이 성공적으로 구현되었습니다.
   * **개선 계획**: 각 도메인의 특성에 맞춰 가중치(Weight) 및 필터링 로직을 더욱 정교하게 튜닝하고, 향후 쇼핑 도메인에 특화된 데이터로 파인튜닝된 커스텀 임베딩 모델을 도입하여 검색 품질을 한층 끌어올릴 예정입니다.
