...
+-----------------------------------------------------------------------------------+
|                             API Gateway (Nginx)                                   |
|  포트: 80 (단일 진입점, 라우팅, Rate Limiting, x-request-id 발급)                 |
+-----------------------------------------------------------------------------------+
                                         |
                  +----------------------+----------------------+
...
### 인프라 (Infrastructure)
* **Nginx 1.25**: API Gateway로서 단일 진입점 역할과 라우팅, Rate Limiting을 수행합니다.
...
* **인프라**: Nginx 1.25 (리버스 프록시)
* **단일 진입점**: 포트 `80`을 통해 모든 API 요청을 수신합니다.
* **주요 기능**:
  * **라우팅**: 클라이언트의 요청 경로에 따라 적절한 마이크로서비스로 트래픽을 전달합니다.
  * **Rate Limiting**: IP당 초당 10회(10 req/s)의 요청 제한을 두어 시스템을 보호합니다.
  * **분산 추적**: 모든 요청에 `X-Request-ID`를 발급/전파하여 OpenTelemetry를 통한 전체 구간 추적을 지원합니다.
  * **타임아웃 처리**: LLM 호출 및 에이전트 루프 처리에 시간이 소요될 수 있으므로 `120초`의 넉넉한 읽기 타임아웃을 설정했습니다.
  * **오류 처리**: 서비스 다운 시 503 상태 코드와 함께 클라이언트 친화적인 커스텀 JSON 에러 페이지를 반환합니다.
...
│       └── pom.xml
├── gateway/                        # API Gateway (Nginx 리버스 프록시)
│   ├── Dockerfile                  # nginx:1.25-alpine 기반
│   └── nginx.conf                  # 라우팅 규칙, Rate Limiting, 타임아웃 설정
├── infra/                           # 인프라스트럭처 설정 파일
...
9. **API Gateway 단일 진입점**
   * **결정**: Nginx를 사용해 모든 클라이언트 요청이 포트 80을 통해 들어오도록 단일 진입점을 구축했습니다.
   * **이유**: 클라이언트 연동을 단순화하고, 분산된 마이크로서비스에 대해 중앙 집중적인 Rate Limiting과 트래픽 라우팅을 수행하며, 전체 구간 추적을 위한 고유 ID를 일관되게 발급하기 위함입니다. 향후 인증 계층 추가에도 유리합니다.
