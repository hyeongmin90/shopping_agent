...
6. **RAG 기반 하이브리드 검색 (RAG-based Hybrid Search)**
   사용자의 자연어 질의를 더 깊이 이해하고 정확한 결과를 반환하기 위해 PostgreSQL pgvector(벡터 및 키워드 통합 검색)를 활용한 하이브리드 검색(RAG)을 지원합니다. OpenAI의 `text-embedding-3-small` 모델을 사용하여 상품, 리뷰, 정책 데이터를 임베딩하고, 각 도메인 특성에 맞는 검색 전략(키워드 우선, 벡터 우선, 균형)을 적용합니다.
   * **동작 방식**: 실시간으로 입력되는 상품과 리뷰 데이터를 자동 임베딩 파이프라인이 처리하여 PostgreSQL(pgvector 및 Full Text Search)에 적재하며, 에이전트가 상황에 맞는 RAG 도구를 호출하여 최적의 검색 결과를 생성합니다.
   * **기대 효과**: "여름에 입기 좋은 시원한 셔츠"와 같은 추상적인 질의부터 특정 약관에 대한 세부 질문까지 사람처럼 이해하고 정확히 답변할 수 있습니다.

### 시스템의 차별점

* **하이브리드 검색 (Hybrid Search)**: 단순 키워드 매칭의 한계를 넘어, 키워드 검색(PostgreSQL fts)과 시맨틱 벡터 검색(PostgreSQL pgvector)을 결합한 하이브리드 RAG 아키텍처를 도입하여 사용자의 의도를 정확하게 파악하고 관련성 높은 정보를 제공합니다.
...
+---------------------------------------------------+                                       | 
                         ...
                         v                                                                  v
+---------------------------------------------------+ +---------------------------------------------------+
|     Postgres pgvector (Vector & Keyword Search)   | |    Embedding Pipeline (Python Kafka Consumer)     |
...
* **asyncpg & pgvector**: 벡터 데이터베이스(PostgreSQL)와의 비동기 통신을 통해 시맨틱 검색과 키워드 검색을 모두 수행합니다.
...
* **PostgreSQL (pg16) + pgvector**: 하이브리드 RAG 검색(벡터 + Full Text Search)을 위한 데이터베이스입니다.
...
* **Keyword Search**: PostgreSQL Full Text Search (`tsvector`)
  * pgvector 확장과 함께 동일한 테이블에서 GIN 인덱스를 활용하여 키워드 매칭 검색을 지원합니다.
...
1. **Product (키워드 우선)**: PostgreSQL의 Full Text Search를 Primary로, 벡터 검색(pgvector)을 Secondary로 활용하며 상품 ID 기준으로 중복을 제거(Dedup)합니다. 상품명, 브랜드 등 정확한 명칭 매칭이 중요하기 때문입니다.
2. **Review (벡터 우선)**: PostgreSQL(pgvector)를 통한 시맨틱 검색만 수행하며 필터링(별점, 검증된 구매자 등)을 결합합니다. 리뷰는 주관적이고 추상적인 표현이 많아 의미 기반 검색이 효과적입니다.
...
* **동작 흐름**:
  * **Product Handler**: 이벤트 수신 → OpenAI API를 통해 임베딩 생성 → PostgreSQL(pgvector 및 fts)에 데이터 Upsert
  * **Review Handler**: 이벤트 수신 → OpenAI API를 통해 임베딩 생성 → PostgreSQL(pgvector 및 fts)에 데이터 Upsert
...
전체 시스템은 `docker-compose.yml`을 통해 API Gateway, Agent Service, 5개의 Spring Boot 마이크로서비스, 그리고 인프라(Kafka, PostgreSQL pgvector, Redis, DB 등)를 포함하여 15개의 컨테이너로 완벽하게 오케스트레이션됩니다.
...
│       │   ├── pgvector_store.py     # PostgreSQL pgvector 및 fts 클라이언트
...
└── tools/seed/                      # 초기 데이터 시드 스크립트
    ...
    ├── qdrant/
    │   └── mock_policies.py             # 쇼핑몰/배송/환불 정책 Mock 데이터 (벡터화 대상)
    └── seed_rag.py                  # PostgreSQL(pgvector + fts) 시드 스크립트 (상품 18개, 리뷰 22개, 정책 10개)
```
...
* **RAG 시드 스크립트 (`seed_rag.py`)**: `docker-compose` 환경 내에서 실행되며, 위에서 언급된 상품, 리뷰, 정책 데이터를 임베딩하여 PostgreSQL 하나에 통합하여 자동으로 적재합니다.
...
| `POSTGRES_AGENT_URL` | `postgresql+asyncpg://agent_user:agent_pass@postgres-agent:5432/agent_db` | Agent 전용 DB 접속 정보 (Vectors & FTS) |
...
8. **키워드와 벡터 하이브리드 검색 전략**
   * **결정**: 상품은 키워드 우선(PostgreSQL fts), 리뷰는 벡터 우선(PostgreSQL pgvector), 정책은 균형 잡힌 방식(0.5/0.5 비율)으로 도메인별 맞춤형 RAG 검색 전략을 적용했습니다.
...
2. **RAG 검색 정확도 튜닝 및 임베딩 모델 고도화**
   * 현재 PostgreSQL 단일 저장소(pgvector 및 fts)를 활용하여 상품, 리뷰, 정책 도메인에 대한 하이브리드 RAG 검색이 성공적으로 구현되었습니다.
