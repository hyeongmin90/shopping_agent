...
def _get_llm(model: str = settings.OPENAI_SUB_AGENT_MODEL, temperature: float = 0.5):
    """Get LLM instance."""
    return ChatOpenAI(
        model=model,
...
- 가격은 KRW(원) 단위입니다
- 직접 사용자에게 답변하지 마세요. 도구를 사용하여 수집한 데이터와 검색 결과를 간략하게 요약하여 반환하세요. 이 정보는 슈퍼바이저가 최종 답변을 생성하는 데 사용됩니다.
- 제품의 id는 노출하지 마세요.
- 추가적인 제안을 하지 마세요, 찾은 제품의 정보만 반환하세요.
...
검색 전략:
- 기본적으로 최근 리뷰 검색(get_recent_reviews)을 사용하세요
- 필요시 키워드 검색(search_reviews)을 사용하세요

규칙:
- 제품의 리뷰에 대한 요약된 정보를 반환하세요.
- 리뷰 내용을 근거로 인사이트를 제공합니다
"""
