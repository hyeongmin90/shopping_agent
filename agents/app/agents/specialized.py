...
검색 전략:
- 키워드 기반 검색(search_products)과 RAG 하이브리드 검색(rag_search_products)을 모두 활용하세요
- 정확한 상품명이나 브랜드가 주어진 경우: search_products(키워드 검색)를 우선 사용하세요
- 추상적/의미적 질문인 경우 (예: '시원한 옷', '가성비 좋은 이어폰'): rag_search_products를 사용하세요
- 두 검색을 조합하여 풍부한 결과를 제공할 수 있습니다
...
- 직접 사용자에게 답변하지 마세요. 주문 처리 결과나 조회된 상태를 상세히 요약하여 반환하세요. 이 정보는 슈퍼바이저가 최종 답변을 생성하는 데 사용됩니다.

중요:
- 구매 승인 없이 절대 주문을 확정하지 마세요
- 장바구니 결제(checkout_cart) 혹은 주문 승인 관련 작업을 할 때 반드시 사용자에게 확인을 받으세요"""
...
def get_agent_tools(agent_type: str) -> list:
    ...
    tool_map = {
        "product_search": PRODUCT_TOOLS + INVENTORY_TOOLS + RAG_PRODUCT_TOOLS,
        "review_analysis": REVIEW_TOOLS + PRODUCT_TOOLS + RAG_REVIEW_TOOLS,
        "cart_management": CART_TOOLS + PRODUCT_TOOLS + INVENTORY_TOOLS + RAG_PRODUCT_TOOLS,
